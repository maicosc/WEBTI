...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        if (!isset($_POST["min"]) || !isset($_POST["max"])) {
            header("Location: atividade5.php?error=valores_faltando");
            exit();
        }
    ...
    $max = (int) $_POST["max"];
    if ($min > $max) {
        header("Location: atividade5.php?error=minimo_maior_que_o_maximo");
        exit();
    } else if ($min == $max) {
        header("Location: atividade5.php?error=minimo_igual_maximo");
        exit();
    } else {
        echo "Aqui está o valor aleatório entre " . $min . " e " . $max . " é " . mt_rand($min, $max) . "";
    }}
    ?>
</body>
